...
| Valid-code checking | none | Open construction. The decision currency is now fixed, but no {lit}`DescriptionCodeValid` predicate or executable well-formedness checker exists, and therefore no exact self-code gate. |
...
currency is now established. The remaining repair requires a valid-code
predicate with an executable checker, complement
