...
{lit}`RegExp` inductive type, and the language generated by an expression
(Definition 3.3) by {lit}`RegExp.Denote`. The primitive constructor equations
of the denotation are definitional: unfolding {lit}`RegExp.Denote` reduces
...
closure constructors are {lit}`RegExp.regular_union`,
{lit}`RegExp.regular_concat`, {lit}`RegExp.regular_star`, and
{lit}`RegExp.finite_language_regular`.
