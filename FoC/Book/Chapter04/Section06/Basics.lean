...
Book: Section 4.6, section body (the definition of general grammars, their
derivations, and the countability of finite presentations).
...
The book's conclusion of the countability paragraph, that languages over a
fixed alphabet are uncountable and hence some language is generated by no
grammar, is not formalized here; only the countability of the presentation
...
closure of yields and derivations under a fixed left and right context; the
example derivations later in the section use them constantly.
-/
