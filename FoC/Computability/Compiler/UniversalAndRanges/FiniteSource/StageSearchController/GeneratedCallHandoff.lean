...
/-!
# Generated stage-code handoff

Shared tape-equivalence lemmas and the generated stage-code handoff
parser used by the StageSearchController generated-call helpers.
-/
...
/--
Public contract for the generated stage-code handoff parser.  The wrapper
halts exactly on inputs that are a generated stage code for which the wrapped
machine halts on that rebuilt stage-coded input.
-/
theorem boundedSimulatorCanonicalInputParserMachine_spec
    (pairRunner : TuringMachine MachineCodeSymbol pairState)
    (tokens : Word MachineCodeSymbol) :
    TuringMachine.HaltsOnInput (BSCIPM pairRunner) tokens <->
      exists budget : Nat,
      exists encoded : Word MachineCodeSymbol,
...
/--
Existential packaging of {name}`boundedSimulatorCanonicalInputParserMachine`.
This is the reusable finite-machine construction for callers that need to
parse a generated stage code and then hand the rebuilt input to a supplied
runner.
-/
theorem boundedSimulatorCanonicalInputParserMachine_construction
    {pairState : Type uStage}
    (pairRunner : TuringMachine MachineCodeSymbol pairState) :
    exists runnerState : Type uStage,
          ...
          exists encoded : Word MachineCodeSymbol,
            tokens = CodePrefixRecognizerStageCode encoded budget ∧
              TuringMachine.HaltsOnInput pairRunner
                (CodePrefixRecognizerStageCode encoded budget) := by
  exact
    ⟨BSCIPS pairState, BSCIPM pairRunner,
      boundedSimulatorCanonicalInputParserMachine_spec pairRunner⟩
