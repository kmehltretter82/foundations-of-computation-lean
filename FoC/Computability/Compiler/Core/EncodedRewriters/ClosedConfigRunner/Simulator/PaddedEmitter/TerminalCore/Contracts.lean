...
This module contains the terminal construction-family contracts, the exact
terminal rewind finite-machine run, and the adapter lemmas that compose the
terminal source, field-FST target, FST target, right-scratch, and final body
layers.  The pure terminal tape-shape facts stay in
