      ...
      (by
        have hlookup :
            CanonicalLayouts.DovetailLayoutScanner.CellSuffixScannerDescription.lookupTransition
              ...
              none = none := by
          native_decide
        simpa [config, tapeAtCells, MachineDescription.stepConfig,
          hlookup, Tape.read])
      (by
        change (10 : Nat) ≠ 99
        omega)
...
theorem configurationSuffixScannerDescription_runConfig_code_handoff
  ...
  have hseq :
      (MachineDescription.seqSubroutine
        CanonicalLayouts.DovetailStagePrefix.NatSuffixScannerDescription
        CanonicalLayouts.DovetailLayoutScanner.TapeSuffixScannerDescription
        Direction.right).runConfig n
          { state :=
              (MachineDescription.seqSubroutine
                CanonicalLayouts.DovetailStagePrefix.NatSuffixScannerDescription
        ...
        { state :=
            (MachineDescription.seqSubroutine
              ...
              Direction.right).halt
          tape := Tout } := by
    simpa [CanonicalLayouts.DovetailLayoutScanner.ConfigurationSuffixScannerDescription,
      config] using h
  rcases
      MachineDescription.seqSubroutine_runConfig_inv
        (A := CanonicalLayouts.DovetailStagePrefix.NatSuffixScannerDescription)
        (B := CanonicalLayouts.DovetailLayoutScanner.TapeSuffixScannerDescription)
        (handoffMove := Direction.right)
        CanonicalLayouts.DovetailStagePrefix.natSuffixScannerDescription_subroutineReady
        CanonicalLayouts.DovetailLayoutScanner.tapeSuffixScannerDescription_subroutineReady
        hseq with
    ⟨Tstage, hstage, htape⟩
  ...
  cases tapeCode with
  | nil =>
      -- Remaining checkpoint: the nat scanner can hand off at the end of the
      -- nat field, so the closed direction still has to rule out a following
      -- tape scanner halt from this empty tape-code suffix.
      sorry
