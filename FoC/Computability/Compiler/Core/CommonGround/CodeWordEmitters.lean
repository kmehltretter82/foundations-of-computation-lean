...
theorem outputCompiled_of_indexed_tape_spec
    {ι : Type}
    {P : TapeCodePrimitive}
    {runner : MachineDescription}
    (hwell : runner.WellFormed)
    ...
    (houtputTape :
      forall i : ι,
        Tape.normalizedOutput (outputTape i) =
          encodeCodeWordAsInput (outputCode i))
    (hforward :
      forall i : ι,
        runner.HaltsWithTape
    ...
    (htransform :
      forall code out : Word MachineCodeSymbol,
        P.transform code = some out <->
          exists i : ι, code = inputCode i ∧ out = outputCode i) :
    TapeCodePrimitiveOutputCompiledSubroutineByDescription P runner := by
  constructor
  · constructor
    · exact hwell
    · intro code out
      ...
      · intro hP
        rcases (htransform code out).mp hP with ⟨i, hcode, hout⟩
        rw [hcode, hout]
        simpa [houtputTape i] using
          haltsWithOutput_of_haltsWithTape
            (hforward i)
  · exact hhaltFree

theorem outputCompiled_of_indexed_tape_equiv_spec
    {ι : Type}
    {P : TapeCodePrimitive}
    {runner : MachineDescription}
    (hwell : runner.WellFormed)
    ...
    (hforward :
      forall i : ι,
        runner.HaltsFromTapeEquiv
          (Tape.input (encodeCodeWordAsInput (inputCode i)))
          (outputTape i))
    (hclosed :
      forall code : Word MachineCodeSymbol,
      forall T : Tape Bool,
        runner.HaltsWithTape
          (encodeCodeWordAsInput code) T ->
        exists i : ι, code = inputCode i ∧ Tape.Equiv T (outputTape i))
    (htransform :
      forall code out : Word MachineCodeSymbol,
        P.transform code = some out <->
          exists i : ι, code = inputCode i ∧ out = outputCode i) :
    ...
    · intro code out
      ...
      · intro hP
        ...
        simpa [HaltsWithOutputIn, HaltsFromTapeWithOutputIn,
          MachineDescription.initial, houtputTape i] using hn
  · exact hhaltFree
