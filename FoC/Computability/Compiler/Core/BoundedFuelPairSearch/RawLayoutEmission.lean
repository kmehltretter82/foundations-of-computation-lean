...
theorem rawEmissionD_haltsWithTapes (attempt : MachineDescription)
    (T0 : Tape Bool) (head : Bool) (tail : Word Bool) (fuel : Nat) :
    (rawEmissionD attempt.start).HaltsWithTapes
  ...
  refine ⟨steps, ?_⟩
  have htarget :
      ({ state := (rawEmissionD attempt.start).halt
         tapes :=
          [ T0
          , rawEmissionFinalScratch (head :: tail) fuel
          , rawEmissionOutputTape attempt (head :: tail) fuel ] } :
        CommonGround.FiniteTransducers.Structured.Configuration) =
        (rawEmissionTable attempt.start).config .halt
          T0 (rawEmissionFinalScratch (head :: tail) fuel)
          (rawEmissionOutputTape attempt (head :: tail) fuel) := by
    rfl
  rw [htarget, rawEmissionD_runConfig_eq_coreD,
    rawEmissionTable_config_eq_coreCfg,
    rawEmissionTable_config_eq_coreCfg]
  exact hrun

/-- Lowered one-tape form of the retargeted generic emitter. -/
theorem rawEmissionLowered_haltsFromTapeEquiv
    (attempt : MachineDescription)
    (T0 : Tape Bool) (head : Bool) (tail : Word Bool) (fuel : Nat) :
    (lowerStructured3Description (rawEmissionD attempt.start))
        |>.HaltsFromTapeEquiv
  ...
  have h :=
    lowerStructured3Description_haltsFromConfigWithTapes
      (rawEmissionTable attempt.start).description_wellFormed
      (rawEmissionTable attempt.start).description_haltTransitionFree
      (rawEmissionTable attempt.start).description_supportsReadWriteRows3
      (c := (rawEmissionTable attempt.start).config (.outFalse0 none)
        T0 (rawEmissionEntryScratch (head :: tail) fuel) Tape.blank)
      (tapes :=
        [ T0
        , rawEmissionFinalScratch (head :: tail) fuel
        , rawEmissionOutputTape attempt (head :: tail) fuel ])
      rfl (by rfl)
      (rawEmissionD_haltsWithTapes attempt T0 head tail fuel)
  simpa [rawEmissionD, encodedGuardedStructured3Tapes,
    TypedStateTable.config, ThreeTape.config] using h

theorem simulatorLayout_asBoolInput_cons (L : SimulatorLayout) :
  ...
  | mk input stage config hit =>
      simpa [SimulatorLayout.asBoolInput, SimulatorLayout.encode,
        SimulatorLayout.encodeAppend] using
        (encodeCodeWordAsInput_cons_head MachineCodeSymbol.header
          (encodeBoolWordAppend input
            (encodeNatAppend stage
              (encodeConfigurationAppend config
                (encodeBoolAppend hit [])))))

/-- Composition-ready specialization: every serialized source-run layout is
nonempty, so it can enter the retargeted generic emitter without a new parser
or case split in the dispatcher. -/
theorem rawEmissionLowered_haltsFromSimulatorLayout
    (attempt : MachineDescription) (T0 : Tape Bool)
    (sourceLayout : SimulatorLayout) (fuel : Nat) :
    exists head : Bool, exists tail : Word Bool,
      SimulatorLayout.asBoolInput sourceLayout = head :: tail ∧
      ((lowerStructured3Description (rawEmissionD attempt.start))
          |>.HaltsFromTapeEquiv
            (encodedGuardedStructured3Tapes T0
              (rawEmissionEntryScratch
                (SimulatorLayout.asBoolInput sourceLayout) fuel)
              Tape.blank)
            (encodedGuardedStructured3Tapes T0
              (rawEmissionFinalScratch
                (SimulatorLayout.asBoolInput sourceLayout) fuel)
              (rawEmissionOutputTape attempt
                (SimulatorLayout.asBoolInput sourceLayout) fuel))) := by
  rcases simulatorLayout_asBoolInput_cons sourceLayout with
    ⟨head, tail, hraw⟩
  refine ⟨head, tail, hraw, ?_⟩
  simpa only [hraw] using
    rawEmissionLowered_haltsFromTapeEquiv
      attempt T0 head tail fuel
