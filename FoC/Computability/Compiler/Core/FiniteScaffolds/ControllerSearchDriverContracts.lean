...
/-!
## Output-indexed fuel-search driver family route

The repaired search surface keeps one subroutine-ready description per
observable Boolean.  It therefore preserves the fuel-search relation without
claiming that one halt-transition-free description can expose incompatible
answers for the same public input.
-/
...
/-!
The legacy scalar routes remain conditional.  The protected invocation
hypothesis supplies output functionality at one fixed limit, but not across
different limits.  Consequently this module intentionally provides no adapter
from the family to a single halt-transition-free scalar searcher without an
explicit cross-limit observable-coherence premise.
-/
