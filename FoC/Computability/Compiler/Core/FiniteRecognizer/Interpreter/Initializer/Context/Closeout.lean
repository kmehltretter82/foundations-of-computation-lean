...
namespace FiniteRecognizer.Interpreter.BooleanContextSavedCloseout
...
open FiniteRecognizer.Interpreter.BooleanContextRawTailDriver
...
def phaseBit
  ...
  | .fourth =>
      FiniteRecognizer.Interpreter.BooleanContextOneSymbolRound.codeSymbolFourthBit symbol
  | .third =>
      FiniteRecognizer.Interpreter.BooleanContextOneSymbolRound.codeSymbolThirdBit symbol
  | .second =>
      FiniteRecognizer.Interpreter.BooleanContextOneSymbolRound.codeSymbolSecondBit symbol
...
def savedTailCells
  ...
  | none => rest
  | some symbol =>
      some (FiniteRecognizer.Interpreter.BooleanContextOneSymbolRound.codeSymbolSecondBit symbol) ::
        some (FiniteRecognizer.Interpreter.BooleanContextOneSymbolRound.codeSymbolThirdBit symbol) ::
        some (FiniteRecognizer.Interpreter.BooleanContextOneSymbolRound.codeSymbolFourthBit symbol) ::
        rest
...
inductive Control where
  | raw (saved : Option MachineCodeSymbol)
      (inner : FiniteRecognizer.Interpreter.BooleanContextRawTailDriver.Driver.Control)
...
def elems : List Control :=
  savedInnerControls
      FiniteRecognizer.Interpreter.BooleanContextRawTailDriver.Driver.Control.finite.elems
      Control.raw ++
  ...
  complete := by
    ...
    | raw saved inner =>
        have h := savedInnerControls_complete
          FiniteRecognizer.Interpreter.BooleanContextRawTailDriver.Driver.Control.finite.elems
          FiniteRecognizer.Interpreter.BooleanContextRawTailDriver.Driver.Control.finite.complete
...
def mapRawAction
    ...
    (Option MachineCodeSymbol × Direction ×
      FiniteRecognizer.Interpreter.BooleanContextRawTailDriver.Driver.Control) ->
...
def transition :
  ...
  | .raw saved inner, read =>
      Option.map (mapRawAction saved)
        (FiniteRecognizer.Interpreter.BooleanContextRawTailDriver.Driver.transition inner read)
  | .rawBounce none, read =>
...
def entry (saved : Option MachineCodeSymbol) : Control :=
  .raw saved FiniteRecognizer.Interpreter.BooleanContextRawTailDriver.Driver.machine.start
...
theorem raw_step_of_some
    ...
    (source target : TuringMachine.Configuration MachineCodeSymbol
      FiniteRecognizer.Interpreter.BooleanContextRawTailDriver.Driver.Control)
    (hstep :
      FiniteRecognizer.Interpreter.BooleanContextRawTailDriver.Driver.machine.stepConfig source =
        some target) :
  ...
  | mk inner tape =>
      ...
      dsimp [FiniteRecognizer.Interpreter.BooleanContextRawTailDriver.Driver.machine] at hstep
      cases htransition :
          FiniteRecognizer.Interpreter.BooleanContextRawTailDriver.Driver.transition inner
            (Tape.read tape) with
      ...
      | some action =>
          ...
          have hnotReady :
              inner ≠
                FiniteRecognizer.Interpreter.BooleanContextRawTailDriver.Driver.Control.finish
                  .gate := by
            intro heq
            subst inner
            simp [FiniteRecognizer.Interpreter.BooleanContextRawTailDriver.Driver.transition,
...
theorem raw_computes_lift
    ...
    (hrun : TuringMachine.Computes
      FiniteRecognizer.Interpreter.BooleanContextRawTailDriver.Driver.machine source target) :
    ...
    machine.runConfigExact? 2
      ...
      some
        { state := Control.locate saved next .fuel
          tape :=
            FiniteRecognizer.Interpreter.BooleanContextOneSymbolRound.Machine.bounceTape
              (Tape.input word) } := by
...
theorem phase_computes_of_tape_equiv
  ...
  · rw [FiniteRecognizer.Interpreter.BooleanContextOneSymbolRound.Machine.layoutWord_eq_prepend_targetWord]
...
theorem full_saved_tail_computes
      ...
      Tape.Equiv
        (Tape.input
          (FiniteRecognizer.Interpreter.BooleanContextOneSymbolRound.Machine.layoutWord
            ...
            []))
        targetTape := by
  rcases FiniteRecognizer.Interpreter.BooleanContextRawTailDriver.Driver.full_tail_computes
  ...
  | none =>
      ...
      · simpa [entry, rawConfig,
          FiniteRecognizer.Interpreter.BooleanContextRawTailDriver.Driver.roundConfig,
          FiniteRecognizer.Interpreter.BooleanContextRawTailDriver.Driver.machine,
          FiniteRecognizer.Interpreter.BooleanContextOneSymbolRound.Machine.popConfig,
          FiniteRecognizer.Interpreter.BooleanContextOneSymbolRound.Machine.machine,
...
end FiniteRecognizer.Interpreter.BooleanContextSavedCloseout
