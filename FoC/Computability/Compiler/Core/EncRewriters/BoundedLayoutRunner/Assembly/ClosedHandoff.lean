...
import FoC.Computability.Compiler.Core.EncRewriters.ClosedCfgRunner.Assembly
...
{name (full := FoC.Computability.EncRewriters.BoundedLayoutRunner.Spec)}`Spec`
for
...
def ClosedHandoffRightShiftedConstruction
  ...
  exists runner : MachineDescription,
    FoC.Computability.EncRewriters.ExactRightShiftedOutputCompiledSubroutineByDescription
      (PairedRecognizerDovetailLayoutCode accept reject)
      runner
...
theorem closedHandoffCompiledSubroutine_of_rightShifted
  ...
  refine ⟨runner, ?_⟩
  exact
    FoC.Computability.EncRewriters.closedHandoffCompiled_of_rightShiftedOutputCompiled
      (FoC.Computability.EncRewriters.rightShiftedOutputCompiledSubroutineByDescription_of_exact
        hrunner)
      (fun htransform => by
        rcases
