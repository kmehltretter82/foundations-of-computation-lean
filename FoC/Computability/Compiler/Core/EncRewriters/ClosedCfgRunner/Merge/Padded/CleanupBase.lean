import FoC.Computability.Compiler.Core.EncRewriters.ClosedCfgRunner.Merge.Padded.Scanners
...
theorem sourceRewindDescription_run_scan_boundary
    (leftBits : Word Bool) (current : Bool)
    (rightCells : List (Option Bool)) :
    sourceRewindDescription.runConfig (leftBits.length + 1)
        { state := 1
...
theorem sourceRewindDescription_run_from_leftStack_boundary_padded
    (leftStack : Word Bool) :
    sourceRewindDescription.runConfig (leftStack.length + 2)
        { state := sourceRewindDescription.start
          tape :=
            DovetailInitialLayoutInitializer.tapeAtCells
              (List.append (leftStack.map some) [none]) [none, none] } =
      { state := sourceRewindDescription.halt
        tape :=
          DovetailInitialLayoutInitializer.tapeAtCells [none]
            (List.append (leftStack.reverse.map some) [none, none]) } := by
...
theorem sourceRewindDescription_haltsFromBoundaryPaddedTape
    (bits : Word Bool) :
    sourceRewindDescription.HaltsFromTape
      (DovetailInitialLayoutInitializer.tapeAtCells
        (List.append (bits.reverse.map some) [none]) [none, none])
      (rewindTargetPaddedTape bits) := by
  have hrun :=
    sourceRewindDescription_run_from_leftStack_boundary_padded
      bits.reverse
  refine ⟨bits.length + 2, ?_⟩
  constructor
  · simpa [rewindTargetPaddedTape] using
...
def SelectedMergePaddedEmitterAfterTransitionSourceCode
...
theorem SelectedMergePaddedEmitterAfterTransitionSourceCode_eq_fields
    (p : SelectedMergeEmitterPayload) :
    SelectedMergePaddedEmitterAfterTransitionSourceCode p =
      MachineCodeSymbol.transition ::
  ...
  simp [SelectedMergePaddedEmitterAfterTransitionSourceCode,
    ParsedLayoutBits, DovetailLayout.encode, DovetailLayout.encodeAppend]
...
theorem SelectedMergePaddedEmitterTargetBits_eq_outputCode
...
theorem SelectedMergeEquivEmitterPaddedOutputTape_normalizedOutput_eq_outputCode
    (useAccept : Bool) (p : SelectedMergeEmitterPayload) :
    Tape.normalizedOutput
        (SelectedMergeEquivEmitterPaddedOutputTape useAccept p) =
      encodeCodeWordAsInput
...
theorem SelectedMergeEquivEmitterPaddedOutputTape_cells_eq_outputCode
    (useAccept : Bool) (p : SelectedMergeEmitterPayload) :
    Tape.cells (SelectedMergeEquivEmitterPaddedOutputTape useAccept p) =
      List.append
        ((encodeCodeWordAsInput
          (SelectedMergeOutputCode useAccept p.S p.L)).map some)
        (List.replicate (SimulatorLayout.asBoolInput p.S).length none) := by
