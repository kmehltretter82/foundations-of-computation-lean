...
{name (full := FoC.Computability.EncRewriters.CanonicalLayouts.DovetailLayoutScanner.CellListSuffixScannerDescription)}`CellListSuffixScannerDescription`, but it rejects payload cells encoded
