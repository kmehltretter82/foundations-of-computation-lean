...
/-- A language recognized exactly by a deterministic finite automaton. -/
def DFARecognizable (L : Language alpha) : Prop :=
  DFA.Recognizable L

/-- A language recognized exactly by a nondeterministic finite automaton. -/
...
/-- An expression for the symbols that move a DFA between two fixed states. -/
...
/-- An expression for paths whose intermediate states come from an allowed list. -/
...
/-- The state-elimination regular expression associated with a DFA. -/
...
/-- Every regular language is DFA-recognizable by determinizing its Thompson NFA. -/
theorem regular_is_dfa_recognizable {L : Language alpha}
  ...
  | intro state hstate =>
      ...
      | intro M hM =>
          ...
          exists NFA.SubsetDFAAuto M
          exact FoC.Foundation.FSet.equal_trans
            (NFA.subsetDFAAuto_language M) hM
...
theorem nfa_subset_construction_auto {state : Type} (M : NFA alpha state) :
    DFARecognizable (NFA.AcceptedLanguage M) :=
  NFA.nfa_language_dfa_recognizable_auto M
...
theorem dfa_recognizable_regular_of_list (alphabet : List alpha)
...
theorem dfa_language_regular_of_list [DecidableEq state]
...
theorem nfa_language_regular_of_list (alphabet : List alpha)
    (halphabet : forall a, a ∈ alphabet)
    {state : Type} (M : NFA alpha state) :
    Regular (NFA.AcceptedLanguage M) :=
  dfa_recognizable_regular_of_list alphabet halphabet
    (nfa_subset_construction_auto M)

theorem nfa_recognizable_regular_of_list (alphabet : List alpha)
    (halphabet : forall a, a ∈ alphabet) {L : Language alpha}
  ...
  | intro state hstate =>
      cases hstate with
      | intro M hM =>
          cases nfa_language_regular_of_list alphabet halphabet M with
          | intro r hr =>
              exists r
              exact FoC.Foundation.FSet.equal_trans hr hM

theorem dfa_recognizable_complement_regular_of_list (alphabet : List alpha)
    (halphabet : forall a, a ∈ alphabet) {L : Language alpha}
    (hL : DFARecognizable L) :
...
theorem dfa_recognizable_intersection_regular_of_list (alphabet : List alpha)
...
theorem dfa_recognizable_difference_regular_of_list (alphabet : List alpha)
    (halphabet : forall a, a ∈ alphabet) {L M : Language alpha}
    (hL : DFARecognizable L) (hM : DFARecognizable M) :
...
theorem complement_regular_of_list (alphabet : List alpha)
    (halphabet : forall a, a ∈ alphabet) {L : Language alpha}
    (hL : Regular L) :
    Regular (Language.Compl L) :=
  dfa_recognizable_complement_regular_of_list alphabet halphabet
    (regular_is_dfa_recognizable hL)

theorem intersection_regular_of_list (alphabet : List alpha)
    (halphabet : forall a, a ∈ alphabet) {L M : Language alpha}
    (hL : Regular L) (hM : Regular M) :
    Regular (Language.Inter L M) :=
  dfa_recognizable_intersection_regular_of_list alphabet halphabet
    (regular_is_dfa_recognizable hL) (regular_is_dfa_recognizable hM)

theorem difference_regular_of_list (alphabet : List alpha)
    (halphabet : forall a, a ∈ alphabet) {L M : Language alpha}
    (hL : Regular L) (hM : Regular M) :
    Regular (Language.Diff L M) :=
  dfa_recognizable_difference_regular_of_list alphabet halphabet
    (regular_is_dfa_recognizable hL) (regular_is_dfa_recognizable hM)

/-!
## Finite-alphabet wrappers

The list-based state-elimination theorems above expose the construction's
actual symbol enumeration.  The canonical book-facing API packages that list
and its coverage proof as a {name}`Foundation.FiniteType` witness.
-/

/-- Every DFA-recognizable language over a finite alphabet is regular. -/
theorem dfa_recognizable_regular (alphabet : Foundation.FiniteType alpha)
    {L : Language alpha} (hL : DFARecognizable L) : Regular L :=
  dfa_recognizable_regular_of_list alphabet.elems alphabet.complete hL

/-- The language of a DFA over a finite alphabet is regular. -/
theorem dfa_language_regular [DecidableEq state]
    (alphabet : Foundation.FiniteType alpha)
    (M : DFA alpha state) [DecidablePred M.accept] :
    Regular (DFA.Language M) :=
  dfa_language_regular_of_list alphabet.elems alphabet.complete M

/-- The language of an NFA over a finite alphabet is regular. -/
theorem nfa_language_regular (alphabet : Foundation.FiniteType alpha)
    {state : Type} (M : NFA alpha state) :
    Regular (NFA.AcceptedLanguage M) :=
  nfa_language_regular_of_list alphabet.elems alphabet.complete M

/-- Every NFA-recognizable language over a finite alphabet is regular. -/
theorem nfa_recognizable_regular (alphabet : Foundation.FiniteType alpha)
    {L : Language alpha} (hL : NFARecognizable L) : Regular L :=
  nfa_recognizable_regular_of_list alphabet.elems alphabet.complete hL

/-- Complement preserves DFA-backed regularity over a finite alphabet. -/
theorem dfa_recognizable_complement_regular
    (alphabet : Foundation.FiniteType alpha) {L : Language alpha}
    (hL : DFARecognizable L) : Regular (Language.Compl L) :=
  dfa_recognizable_complement_regular_of_list
    alphabet.elems alphabet.complete hL

/-- Intersection preserves DFA-backed regularity over a finite alphabet. -/
theorem dfa_recognizable_intersection_regular
    (alphabet : Foundation.FiniteType alpha) {L M : Language alpha}
    (hL : DFARecognizable L) (hM : DFARecognizable M) :
    Regular (Language.Inter L M) :=
  dfa_recognizable_intersection_regular_of_list
    alphabet.elems alphabet.complete hL hM

/-- Difference preserves DFA-backed regularity over a finite alphabet. -/
theorem dfa_recognizable_difference_regular
    (alphabet : Foundation.FiniteType alpha) {L M : Language alpha}
    (hL : DFARecognizable L) (hM : DFARecognizable M) :
    Regular (Language.Diff L M) :=
  dfa_recognizable_difference_regular_of_list
    alphabet.elems alphabet.complete hL hM

/-- Regular languages over a finite alphabet are closed under complement. -/
theorem complement_regular (alphabet : Foundation.FiniteType alpha)
    {L : Language alpha} (hL : Regular L) :
    Regular (Language.Compl L) :=
  complement_regular_of_list alphabet.elems alphabet.complete hL

/-- Regular languages over a finite alphabet are closed under intersection. -/
theorem intersection_regular (alphabet : Foundation.FiniteType alpha)
    {L M : Language alpha} (hL : Regular L) (hM : Regular M) :
    Regular (Language.Inter L M) :=
  intersection_regular_of_list alphabet.elems alphabet.complete hL hM

/-- Regular languages over a finite alphabet are closed under difference. -/
theorem difference_regular (alphabet : Foundation.FiniteType alpha)
    {L M : Language alpha} (hL : Regular L) (hM : Regular M) :
    Regular (Language.Diff L M) :=
  difference_regular_of_list alphabet.elems alphabet.complete hL hM
