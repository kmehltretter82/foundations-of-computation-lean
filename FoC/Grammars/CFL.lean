...
The predicates package “generated by a CFG” and the finite-production version
used for the book's context-free language class.

The plain exported {lit}`ContextFreeLanguage` predicate is the
finite-production version. The broader semantic predicate
{lit}`GeneratedByCFG` remains available when a theorem only needs generation
by an arbitrary CFG production predicate.
-/
